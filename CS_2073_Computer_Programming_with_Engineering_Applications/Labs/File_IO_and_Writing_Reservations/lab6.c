...
int main() {
    ...
    // Open reservations.txt in write mode 
    spOut = fopen("reservations.txt", "w");
    if (!spOut) {
    ...
    for (int i = 0; i < num_groups; i++) {
        ...
        int room_type_num = (num_people <= 2) ? 1 : 2;  // 1 for King, 2 for Queen
        // Group number (index+1), number of occupants, number of nights, room type, discount, total cost
        fprintf(spOut, "%d, %d, %d, %d, %.2lf, %.2lf\n", i+1, num_people, num_nights, room_type_num, discount, total_cost);
